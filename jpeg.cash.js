            ...
            results["result"].map((result) => {
              ...
              if (
                ...
                if (!Number(result.isError)) {
                  ...
                  $("#warning").append(JSON.stringify(result) + "</br>");
                }
              }
      ...
      if (app.api.called.length !== app.api.stack.length) {
        for (x = 0; x < app.api.stack.length; x++) {
          if (!app.api.called.includes(app.api.stack[x])) {
            ...
            $(".message").append(" ----- " + x + " of " + app.api.stack.length);
            break;
          }
        }
      } else {
        ...
        $(".message").append(
          " ----- " + app.api.stack.length + " of " + app.api.stack.length
        );
...
app.finish = () => {
    ...
    uniqueNfts.map((nft) => {
      ...
      if (uniqueCryptos.length == 0) {
        ...
        obj = {
          ...
          standard: "ERC721",
          positionHTML: "<span class='position-open'>" + "Open" + "</span>",
          position: "Open",
          flagFormatted: app.data.spam.includes(nft["contractAddress"])
            ? "<span class='spam'>spam</span>"
            : "",
          flag: app.data.spam.includes(nft["contractAddress"]) ? "spam" : "",
          transaction: "Airdrop",
          floorPrice: "0.00",
          to: nft["to"],
          from: nft["from"],
          ...
          unique: unique,
          fx: app.commas(app.data.history[curDate] || app.data.latest || 0),
        };
        if (obj["tokenSymbol"] == "ETH" || obj["tokenSymbol"] == "WETH") {
          if (
        ...
        uniqueCryptos.map((crypto) => {
          unique = "unique-" + (app.data.unique += 1);
          nft["gasCalc"] =
            ((Number(crypto.gasPrice || 0) * Number(crypto.gasUsed || 0)) /
              uniqueNfts.length /
              1000000000000000000) *
            gasFlag;
          if (gasFlag) {
          ...
          nft["valCalc"] =
            Number(crypto.value || 0) / uniqueNfts.length / 1000000000000000000;

          switch (true) {
            case !app.data.addresses.includes(crypto.to.toLowerCase()) &&
              app.data.addresses.includes(crypto.from.toLowerCase()):
              nft["cryptoInOut"] = "out";
              break;
            case app.data.addresses.includes(crypto.to.toLowerCase()) &&
              !app.data.addresses.includes(crypto.from.toLowerCase()):
  ...
  app.data.dataset.map((nft) => {
    ...
    switch (positionObj[concat_]) {
      case "Closed":
        if (nft["tokenSymbol"] == "ETH" || nft["tokenSymbol"] == "WETH") {
          nft["position"] = "Closed";
          nft["positionHTML"] =
      ...
      case "Open":
        if (nft["tokenSymbol"] == "ETH" || nft["tokenSymbol"] == "WETH") {
          if (!floorUsed.includes(concat_)) {
            if (app.data.pricing.hasOwnProperty(nft["contractAddress"])) {
              floorPrice =
                app.data.pricing[nft["contractAddress"]]["floor_price"] *
                (app.data.latest || 0);
...
app.drawStats = () => {
  ...
  $("#realizedWithFees").html(
    app.settings.prefix +
      app.commas(
        app.round(app.stats.received - app.stats.sent - app.stats.gas, 2)
      ) +
      app.settings.suffix
  );
...
app.commas = function (var_) {
  ...
  switch (num.length) {
    ...
    case 7:
      number_ =
        neg + num.slice(0, 1) + "," + num.slice(1, 4) + "," + num.slice(4, 7);
      break;
    case 8:
      number_ =
        neg + num.slice(0, 2) + "," + num.slice(2, 5) + "," + num.slice(5, 8);
      break;
    case 9:
      number_ =
        neg + num.slice(0, 3) + "," + num.slice(3, 6) + "," + num.slice(6, 9);
      break;
    case 10:
      number_ =
        neg +
...
app.feedback = (feedback) => {
  ...
  const regexp = /^0x[a-fA-F0-9]{40}$/g;
  if (!address.toString().match(regexp)) {
    $("#" + feedback.replace("Confirm", "Message")).html("invalid contract");
    return;
  }
  ...
  fetch(url)
    ...
    .then((data) => {
      if (data.hasOwnProperty("feedback")) {
        ...
        $("#" + data["feedback"].replace("Confirm", "Message")).addClass(
          "success-green"
        );
      } else {
        //
    ...
    switch (param) {
      ...
      default:
        text_ =
          ...
          app.settings.prefix +
          app.commas(
            app.round(Math.abs(txn[0]["value"]), app.settings.decimals)
          ) +
          app.settings.suffix +
          "</strong> for " +
          "<strong>" +
