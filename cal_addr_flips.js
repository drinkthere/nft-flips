...
const finish = async (addr) => {
  ...
  for (hash in app.data.hash) {
    ...
    uniqueNfts.map((nft) => {
      //flow of nft
      switch (true) {
        case addr != nft.to.toLowerCase() && addr == nft.from.toLowerCase():
          nft["nftInOut"] = "out";
          break;
        case addr == nft.to.toLowerCase() && addr != nft.from.toLowerCase():
          nft["nftInOut"] = "in";
      ...
      if (uniqueCryptos.length == 0) {
        ...
        uniqueCryptos.map((crypto) => {
          ...
          nft["valCalc"] = Number(crypto.value || 0) / uniqueNfts.length / 1e18;
          ...
          obj = {
            ...
            valueFormatted: commas(
              round(nft["valCalc"] * weight, app.settings.decimals)
            ),
            gasPaid: nft["gasCalc"],
        ...
        if (nft["tokenSymbol"] == "ETH" || nft["tokenSymbol"] == "WETH") {
          ...
          switch (nft["transaction"]) {
            ...
            case "Receive":
              ...
              app.stats.gas += nft["gasPaid"];
              app.flips[concat_]["receive"] = Math.abs(nft["value"]);
              break;
            case "GasOnly":
              app.stats.gas += nft["gasPaid"];
...
const outputStats = () => {
  ...
  const stats = {
    ...
    unrealizedWithFees: commas(
      round(app.stats.valueOpen - app.stats.sentOpen - app.stats.gasOpen, 3)
    ),
    winningFlips,
    losingFilps,
  };
